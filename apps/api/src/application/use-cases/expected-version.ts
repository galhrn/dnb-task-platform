 ...
 */
/**
 * @param task the task as just read inside the transaction
 * @param expectedVersion the version the caller believes it acted on; absent means the
 *   caller made no claim, and the repository's own guard still applies
 * @throws {VersionConflictError} if the caller's claim no longer matches
 */
