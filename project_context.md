...
### Marketing (`MARKETING`) — final status 2, shipped as the isolated final commit (ADR-008, ADR-013)
...
task_transitions                        -- append-only, source of truth
  ...
  assigned_user_id uuid not null fk → users(id)   -- who received the task
  actor_user_id uuid not null fk → users(id)      -- who performed the transition
  created_at
  index (task_id, created_at), index (actor_user_id)
```
...
| 2 | `AddTransitionActor1755820000000` | `task_transitions.actor_user_id`, its FK and index. Existing rows backfilled from `assigned_user_id` — a best-effort guess for history recorded before the column existed, done once at migration time rather than inferred at read time and presented as fact. |
...
| ADR-017 | **The transition log records an actor separately from the assignee**, self-asserted by the caller | `assigned_user_id` answers "who has it now", which is not "who did this" — they differ on every handover, and a `CLOSE` hands the task to nobody at all, so the log could not answer "who closed this?". Deriving the actor from the previous holder was rejected: the API lets anyone move anyone's task, so the inference is wrong exactly when an audit matters. | Every mutation now requires `actorUserId`, including close. Because there is no authentication (§2), it is **provenance, not proof** — a client asserts who it is and the server believes it. Documented as such rather than presented as an audit trail. Adding auth later means deriving the actor from the session instead of the body; nothing else moves. |
...
- [x] **M7 — Marketing type.** Isolated commit. *DoD: diff touches exactly two server files and nothing else.* — **done 2026-08-22**; two files, 35 insertions, 226 tests passed unedited. Landed as the pure commit plus a separate `docs(m7)`, since the log rule cannot apply inside a commit whose whole point is what it does not contain.
...
| 2026-08-22 | *Retrospective (docs only):* the MVP data-type simplifications catalogued in `milestone_logs.md` — five of six fields are `string` with `minLength: 1`, and the vocabulary has no format, enum, money, reference or file kind. | Raised in UAT about `quotes`. Documented rather than changed: the assignment asks whether a task *type* can be added without touching existing code, not whether the field vocabulary is rich. Records the distinction that matters — adding a kind is compile-checked and touches no task type, while changing a field already in production is a JSONB data migration (the other half of ADR-007). |
| 2026-08-22 | *Finding, not fixed:* `initialValues()` in `DynamicFieldForm.tsx` uses a `switch` with `default:`, so a new `FieldKind` gets a silently wrong initial value with no compile error — unlike the schema builder and the renderer, which are exhaustive mapped types. | Latent: every current kind is fine with `''` or handled explicitly. Left as a code change for its own commit rather than folded into a docs commit. |
| 2026-08-22 | *Verified:* `campaignUrl` accepts `"not a url"` and `"x"`; only whitespace is rejected. | Checked against the compiled schema rather than asserted, since the retrospective states it as fact. |
| 2026-08-22 | *UI polish:* "Acting as" → "Current user"; history shows the actor; typography given a consistent weight scale and more generous spacing. Backend gained `actor_user_id` (ADR-017, migration 2). | The history could not answer "who did this?" — only "who has it now". Those differ on every handover, and on a close there is no new holder at all. Minimal-UI constraints held: system fonts only, no animations, no icon library. |
| 2026-08-22 | *Consequence:* `POST /tasks/:id/close` now requires a body. | It carries `actorUserId`, so the "close with no body" case became a 400. The integration test that asserted the empty-body case now asserts both halves. |
| 2026-08-22 | **M7 executed** on `feat/m7-marketing`. Marketing added: one new file, one line in the registration list. 226 tests passed unedited; `purity.test.ts` picked the new file up by itself. | Extensibility milestone — the proof, not the claim. |
| 2026-08-22 | *Finding:* the first attempt broke **five assertions**, so the two-file claim was false as the repo stood. Fixed in its own prior commit (`test: decouple tests from which task types happen to be registered`). | Two independent faults: `'MARKETING'` used as the fixture for "a type that is not registered" (a fixture that expires the day the type ships), and `toEqual(['PROCUREMENT','DEVELOPMENT'])` pinning the catalogue inside suites that are about a use case and an endpoint. Kept as a separate, earlier commit on purpose: a diff is only evidence if the tests it passes were not adjusted to let it through, so the adjusting had to be visible on its own terms. |
| 2026-08-22 | *Lesson recorded:* a claim about how little a change costs is also a claim about what the tests are coupled to. | Assertions pinning a registry's contents, in suites not about the registry, tax every future addition — and you only discover it on the day you make one. |
